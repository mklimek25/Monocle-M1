...
# CHANGED (Event Bus): shared bus instance is created here and injected into components.
...
class MonocleCommandCenter:
    def __init__(self, camera_parameters):
        ...
        # CHANGED (Event Bus): create one shared bus for the application.
        self.event_bus = EventBus()

        self.camera = CameraClass(camera_parameters)

        # CHANGED (Event Bus): inject bus into publishers/subscribers.
        self.frame_processor = FrameProcessor(camera_parameters, event_bus=self.event_bus)
        self.data_manager = DataManager(camera_parameters, event_bus=self.event_bus)
        self.gui = GUIManager(self.root, camera_parameters, event_bus=self.event_bus)
    ...
    def initialize_CS_callbacks(self):
        ...
        # CHANGED (Event Bus): this link is now handled by PROCESSING_COMPLETE subscription in DataManager.
        # Keeping callback wiring is optional during migration; you can remove once event-bus is confirmed.
        ...
        # CHANGED (Event Bus): this link is now handled by IMAGE_READY subscription in GUIManager.
        ...
        # CHANGED (Event Bus): this link is now handled by SCAN_ERROR subscription in GUIManager.
