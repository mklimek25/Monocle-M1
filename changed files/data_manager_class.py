...
# CHANGED (Event Bus): DataManager subscribes to PROCESSING_COMPLETE events instead of being called directly.
from event_bus import EventBus, Event, EventType


class DataManager:
    def __init__(self, params, event_bus: EventBus | None = None):
        """
        ...
        # CHANGED (Event Bus): store and subscribe.
        self.event_bus = event_bus
        if self.event_bus is not None:
            # When FrameProcessor publishes results, this handler will receive them.
    ...
    # CHANGED (Event Bus): event handler that bridges event payload -> existing method.
    def _on_processing_complete(self, event: Event) -> None:
        """
        Event handler for results published by FrameProcessor.

        NOTE: We intentionally call the existing `receive_frame_processor_data` so we don't duplicate logic.
        """
        try:
            row_data = event.data
            self.receive_frame_processor_data(row_data)
        except Exception as e:
            # IMPORTANT: don't raise here; EventBus swallows exceptions but we can still log context
            print(f"[DataManager] failed handling {event.event_type.value}: {e!r}")
